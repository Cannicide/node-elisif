...
class TextChannelUtility extends StructureUtility {
     ...
     */
    simpleReactMenu(question, reactions, message = this.lastMessage, time, allUsers) {
        return new Promise((resolve, reject) => {
            // @ts-ignore
     ...
     */
    paginate(options, elements, message = this.lastMessage, perPage) {
    ...
    async button(embedOptions, btnArr) {
        //Can only be implemented once ButtonUtility is finished
    }
    ...
    async selectMenu(embedOptions, selArr) {
        //Can only be implemented once SelectUtility is finished
    }

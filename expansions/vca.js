  ...
  commands: [
    new Command({
      ...
      async execute(message) {
        ...
        if (!message.member.voice.channel) return util.channel(message.channel, message).embed({desc: `**You must be in a voice channel to use VCA!**`});


        var m = await util.channel(message.channel, message).selectMenu({
          ...
          options: [
              {
                  ...
                  value: "youtube",
                  emoji: util.message(message).getGlobalSetting("vca.emotes.youtube")
              },
              {
                  label: "Poker Night",
                  description: "Introduce your VC members to your poker face.",
                  value: "poker",
                  emoji: util.message(message).getGlobalSetting("vca.emotes.poker")
              },
              {
                  label: "Betrayal.io",
                  description: 'Play a "social deduction" game with VC members.',
                  value: "betrayal",
                  emoji: util.message(message).getGlobalSetting("vca.emotes.betrayal")
              },
              {
                  label: "Fishington.io",
                  description: "Fish with VC members. Just how bored are you...?",
                  value: "fishing",
                  emoji: util.message(message).getGlobalSetting("vca.emotes.fishing")
              },
              {
                  label: "Chess",
                  description: "Play chess online or with a VC member.",
                  value: "chess",
                  emoji: util.message(message).getGlobalSetting("vca.emotes.chess")
              }
          ]
        });

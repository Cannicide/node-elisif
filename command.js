 ...
 */
function Command(name, {perms = false, roles = false, invisible = false, desc = "", dm_only = false, cooldown = false, aliases = false, channels = false, isalias = false, args = false}, method) {
    var message = false;
    var prefix = false;
    var timestamps = [];
    var flags = false;
    ...
    channels.forEach(channel => {
      if (!(channel in channelCommandMap)) channelCommandMap[channel] = [];
    ...
    this.execute = function(userArgs) {
        ...
        if (!message) {
          ...
          missedArgs.forEach(arg => {
            ...
            if (feedback != "none" && typeof error == "string") error += feedback + "\n";

            //Allow attaching Embed feedback.
            if (typeof feedback == "object") error = feedback;
          });
    ...
    if (name && method) commands.push({name: name, cmd: this, special: invisible, desc: desc, dm_only:dm_only, cooldown:cooldown, channels:channels, aliases: aliases, isalias: isalias, perms: perms, roles: roles, flags: flags});

    if (name && method && !isalias) aliases.forEach(alias => {
